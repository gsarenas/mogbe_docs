...
| Robô (sim) + controlador + mundo + posição | Dev | `ros2 launch mogbe mogbe_simulation.launch.py use_ros2_control:=<bool> world:=</path/to/world_name.world> x:=<float> y:=<float> z:=<float>` |
...
| Carrega e ativa diff_drive_controller | Dev/Pi | `ros2 run controller_manager spawner diff_cont` |
| Carrega e ativa joint_state_broadcaster | Dev/Pi | `ros2 run controller_manager spawner joint_broad` |
